...
-- Functions
CREATE OR REPLACE FUNCTION GetImmigrantAge(passport VARCHAR(50))
RETURNS INT AS $$
DECLARE
    age INT;
BEGIN
    SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, DOB)) INTO age
    FROM Immigrants
...
CREATE OR REPLACE FUNCTION CheckVisaStatus(passport VARCHAR(50))
...
BEGIN
    ...
    IF expiry IS NULL THEN
        result := 'Not Found';
    ELSIF expiry >= CURRENT_DATE THEN
        result := 'Valid';
    ELSE
        result := 'Expired';
    END IF;
...
CREATE OR REPLACE FUNCTION ImmigrantCountByCountry(country_name VARCHAR(100))
RETURNS INT AS $$
DECLARE
    count_result INT;
BEGIN
...
CREATE OR REPLACE FUNCTION SponsorshipDuration(employee_passport VARCHAR(50))
RETURNS INT AS $$
DECLARE
    duration INT;
BEGIN
    SELECT (Employee_End_Date - Employee_Start_Date) INTO duration
    FROM Sponsors_Employee
    WHERE Employee_Passport_No = employee_passport;
    RETURN duration;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION GetOfficerDepartmentName(eid INT)
RETURNS VARCHAR(100) AS $$
DECLARE
    dep_name VARCHAR(100);
BEGIN
    SELECT d.Name INTO dep_name
    FROM Department d
    JOIN Govt_Employee g ON g.E_ID = eid
    JOIN Visa_Officer v ON v.E_ID = g.E_ID
    WHERE d.DID = v.DID
    LIMIT 1;
    RETURN dep_name;
END;
